...
pub fn traverse_tree_batch(
    ...
    let mut file_count = 0; // Record the number of files scanned
    let mut batch_files = Vec::new(); // Temporary storage for batch files' paths and contents
    ...
    for entry in tree.iter() {
        ...
        if entry.kind() == Some(git2::ObjectType::Blob) {
            ...
            batch_files.push((entry_path, content.to_string()));
            file_count += 1;

            // If the batch size is reached, add the batch files to the total files list
            if file_count >= batch_size {
                files.extend(std::mem::take(&mut batch_files));
                file_count = 0;
            }
    ...
    files.extend(batch_files); // Add the remaining batch files to the total files list
